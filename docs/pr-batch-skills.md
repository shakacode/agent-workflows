...
4. Record `Launch assurance` separately from every `Worker model/effort route`:
   ...
   Signed-launch readiness is separate from install freshness. A clean Codex host with all three host-owned capability/trust files absent is typed `unsupported`; a complete safe manifest plus both public-only RSA anchors of at least 2048 bits is `supported`; partial, malformed, replaced, symlinked, or writable state is `UNKNOWN` and stays blocked. Only exact `unsupported` may use an `agent-workflow-bootstrap-waiver` version 1: a durable safe file recording direct in-session human approval for the exact issue supplied as non-`UNKNOWN` `expected_issue`, batch, lane, hard dispatcher, model, effort, and no fallbacks, plus dispatcher-bound, instance-bound, explicit non-inherited live request metadata for the exact assignment. Initial activation and every `waived-active` replay require an observation bounded to 300 seconds past/future and following a non-future grant. This persists `waived-active`; when signed support later appears, a valid host-signed confirmation transitions the same assignment to `confirmed-active` and removes its waiver binding. Until then replay is a liveness heartbeat, so after the prior observation ages out the coordinator must re-observe the host and resupply a fresh exact assignment-bound observation for the unchanged durable record and exact issue. Caller-supplied persisted state or wrapper digests do not extend freshness. Caller ownership, environment trust, generated keys, synthetic signatures, inherited routing, fallback dispatchers, or broader gate bypasses do not qualify.
