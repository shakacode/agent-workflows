...
| `$evaluate-issue`    | An already-filed issue's value, priority, or proposed fix scope is uncertain.                               | A disposition: fix now, fix later, park, document/work around, close, or ask.         |
| `$spec`              | The user has vague feature or bug intent with no concrete GitHub issue yet.                                 | A traceable spec plus executable tasks ready for `$plan-pr-batch`.                    |
