...
| Return and review | At the availability time, refresh GitHub rather than trusting cached handoffs or workflow evidence. Review current heads, checks, reviews, unresolved threads, dependencies, and merge state across the active PRs; for each wholly non-GitHub trusted-ad-hoc run, also refresh its existing durable plan or backend destination. Then update the durable evidence and select the next integration or decision. | A live PR and durable-run view, an ordered decision queue, and a current integration choice. |
| Walk through or retarget | Refresh the exact diff before using the prepared walkthrough map; rebuild stale sections, then publish the orientation and every conceptual section to the PR in one pass. Publish one COMMENT-only review with one separately replyable inline thread per concept, each on an honest changed line. If no honest anchor exists, state the limitation in the review body and stop instead of claiming complete coverage. Let the owning task consume replies asynchronously. Use a live one-section-at-a-time walkthrough only when the maintainer explicitly asks. Apply simple non-safety overrides at the next safe checkpoint. | The operator gets a prepared, current GitHub walkthrough or an immediately visible new target, return time, queue order, hold, or priority. |
...
- Use [`$pr-walkthrough`](../skills/pr-walkthrough/SKILL.md) to build the
  complete conceptual map, refresh its diff identity, and publish all sections
  as an asynchronous GitHub walkthrough.
