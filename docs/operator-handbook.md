...
| Filtered target confirmation | When a batch resolves targets from filters instead of exact numbers, the resolved list comes to you before any branch or worker is created, unless you explicitly say to proceed without confirmation. | [Target Resolution Gate](../workflows/pr-processing.md#target-resolution-gate) |
| No-backend single-operator approval | When the configured private backend is unavailable and no coordination target is supplied, an ad-hoc lane needs your explicit approval before it can branch. | [Coordination State](../workflows/pr-processing.md#coordination-state) |
...
| QA waivers and visual evidence | A hosted QA waiver is a separately authenticated human decision. GitHub CLI 2.99.0+ can upload PR evidence with `--attach` on GitHub.com and GitHub Enterprise Cloud, but not GitHub Enterprise Server in this release; readiness stays blocked until a human attaches it only when neither the CLI, browser, nor another configured uploader is usable. | [Hosted Runtime QA Gate](../workflows/pr-processing.md#hosted-runtime-qa-gate), [Durable Visual Evidence Gate](../workflows/pr-processing.md#durable-visual-evidence-gate) |
| Hosted-CI skip approval | The `skip-hosted` route requires your explicit maintainer approval. | [Hosted CI Backpressure](../workflows/pr-batch-integration-closeout.md#hosted-ci-backpressure) |
