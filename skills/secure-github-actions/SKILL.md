...
from `.agents/agent-workflow-operational.yml` when that file exists, falling
back to `.agents/agent-workflow.yml` for older consumers. The sidecar is
authoritative when present, including when it contains an empty list; malformed
sidecar policy fails closed. Entries are unique, case-insensitive exact
repository identities. Wildcards, organization-wide trust, refs, subpaths,
aliases, and `UNKNOWN` are invalid. Allowlisting never waives the full-SHA or
readable-version-comment rules.
