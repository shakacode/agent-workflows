...
2. Use existing scope and available task inventory. Record exact parent IDs and
...
3. Ask each affected parent to stop starting new work, checkpoint its own live
   ...
   Send to parents, not their child task IDs. Do not create replacement towers.
...
```text
Prepare for restart by <ABSOLUTE UTC DEADLINE>. Forward this same deadline to
...
preparation-incomplete with the specific exceptions. Then stop work.
```
...
For `fleet-resume`, re-list the specified host's parents and use existing IDs.
Dispatch this recovery to eligible parents; do not resume complete, intentionally
paused or expired tasks without separate authorization. Wait in bounded
snapshots. Isolate uncertain tasks while independent verified tasks recover.
Report per-parent evidence and next action; do not retry a recurring failure
indefinitely or send recovery prompts directly to child threads.
