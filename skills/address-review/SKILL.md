...
- After a successful private claim on an issue/PR, mirror it to the seam's claim
  ...
  the same as the batch claim step — mirror only when the backend provides claim-label
  expiry reconciliation, and skip entirely when `coordination_backend: n/a`.
