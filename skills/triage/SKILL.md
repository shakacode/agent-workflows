...
5. Produce one target-specific pr-batch goal prompt per group, with a stable
   ...
   Render the exact Base and `ask` mechanics for the selected target: `$pr-batch` and `$pr-walkthrough` for Codex, `/pr-batch` and `/pr-walkthrough` for Claude, or neutral `pr-batch` and `pr-walkthrough` names for portable.
