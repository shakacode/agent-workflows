...
5. Produce one target-specific `$pr-batch` goal prompt per group, with a stable
   ...
   Build and reconcile registration provenance through the loaded pack's [canonical Batch Provenance Manifest](../../docs/coordination-backend.md#batch-provenance-manifest). That contract owns requested-versus-observed routes, field-granular `UNKNOWN`, capability discovery, bounded safe invocation, and non-wedging failure behavior.
