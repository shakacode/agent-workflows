...
class PostMergeAuditPolicyTest < Minitest::Test
  ...
  REQUIRED_RECEIPT_HELPER_RULE = "Use `completed-batch-audit-receipt` for `publish`, `update`, and `replay`; `--targets-json` is a JSON array of exact `host`, `repo`, `type` (`pull_request` or `issue`), and positive `number` objects."
  ...
  REQUIRED_CONCISE_RECEIPT_TERMINAL_UPDATE_RULE = "The helper rejects caller-supplied `prior_receipt_sha256`; replay accepts an edited receipt only when authenticated GitHub edit history proves the original single-follow-up blocked body and exactly one same-author terminal edit whose original-body hash matches `prior_receipt_sha256`."
  ...
  def test_issue_result_projection_and_one_time_receipt_update_are_documented
    ...
    detailed_paths.each do |relative_path|
      ...
      assert_includes text, REQUIRED_RECEIPT_TERMINAL_UPDATE_RULE,
                      "#{relative_path} should permit only one bound in-place blocked-receipt terminalization"
    end
    ...
    concise_paths.each do |relative_path|
      ...
      assert_includes text, REQUIRED_CONCISE_RECEIPT_TERMINAL_UPDATE_RULE,
                      "#{relative_path} should preserve concise authenticated terminalization rules"
    end
  end
end
