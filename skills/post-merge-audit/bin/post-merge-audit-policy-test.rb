...
class PostMergeAuditPolicyTest < Minitest::Test
  ...
  REQUIRED_RECEIPT_HELPER_RULE = "Use `completed-batch-audit-receipt` for `publish` and `replay`; the retained `update` command fails closed before any GitHub read or mutation and directs the coordinator to publish a new immutable receipt. `--targets-json` is a JSON array of exact `host`, `repo`, `type` (`pull_request` or `issue`), and positive `number` objects."
  ...
  REQUIRED_RECEIPT_TERMINAL_UPDATE_RULE = "Receipt comments are immutable. When a blocked receipt's follow-up later clears, leave that receipt unchanged and publish a new clean receipt after fresh preflight. The retained `update` command fails closed before any GitHub read or mutation; caller-supplied `prior_receipt_sha256` is rejected and is not valid marker grammar. Replay rejects every edited receipt because GitHub documents `UserContentEdit.diff` only as a change summary, not a complete historical body."
  ...
  REQUIRED_CONCISE_RECEIPT_TERMINAL_UPDATE_RULE = "Receipt comments are immutable: `update` fails before GitHub access, caller-supplied `prior_receipt_sha256` is invalid, and replay rejects every edited receipt because GitHub edit diffs are not documented historical bodies."
