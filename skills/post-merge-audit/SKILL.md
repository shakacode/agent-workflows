...
Use `completed-batch-audit-receipt` for `publish` and `replay`; the retained `update` command fails closed before any GitHub read or mutation and directs the coordinator to publish a new immutable receipt. `--targets-json` is a JSON array of exact `host`, `repo`, `type`
