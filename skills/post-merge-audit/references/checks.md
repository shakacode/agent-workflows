...
- Changelog: if the diff or PR body indicates a user-visible behavior, API, error message, configuration, performance, security, or breaking change, verify the repo's changelog (see `changelog` in `.agents/agent-workflow.yml` or the consumer's `AGENTS.md` when `.agents/agent-workflow.yml` has top-level `version: 1`) has a matching entry. When entries are missing, recommend running `/update-changelog`.
