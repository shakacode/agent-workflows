...
guarded_source_fetch = %r{if \[ -n "\$\{SOURCE_PR_NUMBER\}" \]; then\n\s+"\$\{ADDRESS_REVIEW_SKILL_DIR\}/bin/fetch-pr-review-data" "\$\{SOURCE_PR_NUMBER\}" --repo "\$\{REPO\}" --trust-config "\$\{TRUST_CONFIG_PATH\}" --expected-trust-digest "\$\{TRUST_CONFIG_DIGEST\}" > source-review-data\.json}
