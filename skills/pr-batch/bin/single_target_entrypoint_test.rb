...
assert(batch.include?("For one direct-prompt task, use the\n   derived `adhoc:<yyyymmdd>-<short-slug>` target"), "the required interview must accept ad-hoc targets")
...
assert(batch.include?("**Targets**: for GitHub issue/PR work"), "the required interview must distinguish GitHub targets")
assert(batch.include?("the exact target\n   `Linear issue <ID>: <verified Linear URL>`"), "the required interview must accept the exact Linear target grammar")
assert(batch.include?("untrusted Linear verification is literal\n   `UNKNOWN` and stops before launch"), "the required interview must fail closed on unverified Linear targets")
assert(batch.include?("Extract only explicit GitHub PR/issue refs, verified Linear entries"), "continuation extraction must accept verified Linear targets")
assert(batch.include?("trusted persisted\ncanonical ad-hoc entries in the exact form `adhoc:<yyyymmdd>-<short-slug>`"), "continuation extraction must accept trusted canonical ad-hoc targets")
assert(batch.include?("missing,\nmismatched, unavailable, or untrusted verification is literal `UNKNOWN` and\nstops continuation"), "continuation extraction must retain fail-closed Linear verification")
assert(batch.include?("Refresh each extracted target from its target-specific live\nsource"), "continuation must refresh target-specific live state")
assert(!batch.include?("Continue from live GitHub state;"), "continuation must not keep a GitHub-only refresh rule")
...
assert(batch.include?("structured public claim comments are advisory fallback state only for GitHub"), "pr-batch coordination summary must qualify GitHub public fallback surfaces")
assert(batch.include?("verified Linear and\nad-hoc lanes have no GitHub public comment or label surface"), "pr-batch coordination summary must exclude Linear/ad-hoc public surfaces")
assert(batch.include?("A GitHub issue/PR\nlane claim also mirrors to the seam's claim label"), "pr-batch coordination summary must qualify GitHub label mirroring")
assert(batch.include?("do not publicly mirror verified\nLinear or ad-hoc lane claims"), "pr-batch coordination summary must prohibit Linear/ad-hoc public mirroring")
assert(plan.include?("--target <GitHub-issue-or-PR-number|verified-Linear-ID|derived-adhoc-target> --json"), "plan coordination status must accept GitHub, Linear, and ad-hoc targets")
assert(plan.include?("pass the GitHub\n     issue/PR number, verified Linear ID, or derived\n     `adhoc:<yyyymmdd>-<short-slug>` target exactly"), "plan coordination status must pin each target representation")
assert(!plan.include?("--target <issue-or-pr> --json"), "plan coordination status must reject the stale GitHub-only placeholder")
