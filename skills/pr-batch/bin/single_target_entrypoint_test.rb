...
coordination = read_repo_file("workflows/pr-batch-coordination-observability.md")
coordination_normalized = coordination.gsub(/\s+/, " ")
...
assert(workflow.include?("pr-batch-coordination-observability.md"), "processing entrypoint must route to canonical coordination")
assert(coordination.include?("claim --help"), "canonical coordination must preserve claim capability detection")
assert(coordination.include?("heartbeat --help"), "canonical coordination must preserve heartbeat capability detection")
assert(coordination.include?("--thread-handle"), "canonical coordination must preserve extended lane metadata")
assert(coordination.include?("`coordination_backend: n/a`"), "canonical coordination must define no-backend single-target behavior")
assert(coordination_normalized.include?("single-operator assumption in the Lane Card and final handoff"), "no-backend mode must preserve its assumption in evidence")
...
assert(coordination_normalized.include?("public claim fallback is unavailable because there is no issue or PR comment surface"), "canonical coordination must handle ad-hoc lanes without a public claim surface")
assert(coordination_normalized.include?("coordination target or explicit no-backend single-operator approval"), "ad-hoc degraded coordination must stop for a safe ownership decision")
