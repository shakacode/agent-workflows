...
PATH_EXPANSION_DEFAULT =
  ...
  "accepts the path as disjoint. Under maximum-concurrency-one serialization, acceptance alone does not authorize " \
  "resume: the current holder must release the slot, the requester must durably transition out of `blocked`, a " \
  "fresh preflight must accept, and the requester must be absent from `launch.held_lane_ids`; when launch or " \
  "relaunch is needed, it must also be present in `launch.eligible_lane_ids`. The reservation persists until " \
...
WORKER_SUBAGENT_COORDINATION =
  ...
  "lifecycle state, reruns `batch-plan-preflight`, and resumes only after an accepted disjoint result. Under " \
  "maximum-concurrency-one serialization, acceptance alone does not authorize resume: the current holder must " \
  "release the slot, the requester must durably transition out of `blocked`, a fresh preflight must accept, and " \
  "the requester must be absent from `launch.held_lane_ids`; when launch or relaunch is needed, it must also be " \
  "present in `launch.eligible_lane_ids`. The reservation remains active until the verified file-touch map reflects the " \
