...
Every typed target repository has exactly two ASCII `[A-Za-z0-9][A-Za-z0-9._-]*` components, neither exactly `UNKNOWN`; parseable authorization-reference `N` values are positive decimals matching `[1-9][0-9]*`.
...
Dispatch <lane>:preferred <dispatcher>@<route>;fallback <dispatcher>@<route>->...|none;auth <y|n>;ordinary pending/active lifecycle
...
Coord:stable ids+heartbeats;register before launch when supported;claim refusal=>stop;push holder/generation check;known deps=>gate;missing/UNKNOWN=>stop
...
Every target handoff repeats the repository-qualified canonical launch identity
from plan/preflight and claim evidence. A durably overridden ad-hoc handoff also
repeats the accepted override name, trusted authorizer, durable authorization
reference, original task identity, and unchanged stable coordination identity.
A missing, changed, duplicate, or `UNKNOWN` identity is a non-ready blocker.
