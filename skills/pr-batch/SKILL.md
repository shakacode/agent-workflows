...
canonical digest and exact durable non-self-attested URI reference. The batch
...
that lane, and the equation must balance exactly. The first accepted window
binds batch/coordinator/lane identities and roots. Later windows must be
contiguous at the prior exclusive cutoff. Exact replay does not recount; reject
...
already-admitted in-flight boundary per scope and accepted usage interval, and
