...
2. RED: write one failing test.
   - Run the repo's relevant test command from the `AGENTS.md` **Agent Workflow Configuration** seam.
...
- When a direct automated regression test is not practical, document why, then use the closest useful local verification from the repo's `AGENTS.md` seam before and after changing behavior.
- Before handoff or PR creation, run the repo's pre-push local validation from the `AGENTS.md` seam in addition to the targeted tests used during the loop.
