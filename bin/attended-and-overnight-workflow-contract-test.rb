...
DOC = "docs/attended-and-overnight-workflow.md"
...
class AttendedAndOvernightWorkflowContractTest < Minitest::Test
  ...
  def test_the_guide_uses_the_canonical_doc_link_validator
    assert_empty ValidateDocLinks.validate(ROOT, documents: [DOC]),
                 "docs/attended-and-overnight-workflow.md should reuse the canonical doc-link validator"
  end

  def test_the_guide_has_no_absolute_github_dot_com_links
    absolute_github_links = @doc.scan(/\[[^\]]*\]\(([^)\s]+)\)/).flatten.select { |target| target.include?("github.com") }
  ...
  def test_overrides_require_verified_authority_and_treat_github_comments_as_untrusted
    prose = extract_clause(normalize_prose(@doc), starts_with: "Apply plain instructions only after verifying authenticated operator or")
  ...
  def test_unavailable_telemetry_is_preserved_as_unknown_and_stays_nonblocking_alone
    prose = extract_clause(normalize_prose(@doc), starts_with: "When telemetry is unavailable,")
