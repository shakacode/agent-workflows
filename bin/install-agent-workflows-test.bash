...
write_native_scw_state() {
  ...
  install -m 0644 "$ROOT/docs/host-adapter/contract.md" \
    "$plugin_root/docs/host-adapter/contract.md"
  if [[ "$host" = "codex" ]]; then
...
test_plugin_companion_missing_contract_fails_before_mutation() {
  ...
  for host in codex claude; do
    ...
    write_native_scw_state "$host" "$target"
    plugin_root="$target/plugins/cache/agent-workflows/scw/0.1.0"
    rm "$plugin_root/docs/host-adapter/contract.md"
    cp "$target/.agent-workflows-install.json" "$tmp/metadata.before"
    metadata_before="$tmp/metadata.before"
    ...
    [[ "$status" -ne 0 ]] || fail "$host companion install accepted a plugin without the prompt host adapter contract"
    assert_contains "$output" "required prompt host adapter contract"
    for skill in "$ROOT"/skills/*; do
      [[ -d "$skill" ]] || continue
      assert_file "$target/skills/$(basename "$skill")/SKILL.md"
    done
    cmp -s "$metadata_before" "$target/.agent-workflows-install.json" || \
      fail "$host missing-contract failure changed install metadata"
    [[ ! -e "$target/.agent-workflows-install.lock" ]] || fail "$host missing-contract failure retained the install lock"
    [[ ! -e "$target/.agent-workflows-migration-staging" ]] || \
      fail "$host missing-contract failure created a staging receipt"
  done
}
...
test_final_contract_verification_race_rolls_back_before_metadata_commit() {
  local tmp target plugin_root injection counter output status skill
  tmp="$(mktemp -d)"
  target="$tmp/codex-home"
  plugin_root="$target/plugins/cache/agent-workflows/scw/0.1.0"
  injection="$tmp/final-contract-race.rb"
  counter="$tmp/check-count"
  "$ROOT/bin/install-agent-workflows" --host codex --target "$target" --delivery-mode flat >"$tmp/flat.out"
  write_native_scw_state codex "$target"
  cat > "$injection" <<'RUBY'
...
if ARGV.first == "check" && ENV["QA_CHECK_COUNTER"]
  ...
  if count == 3
    target = ARGV[ARGV.index("--target") + 1]
    FileUtils.rm_f(File.join(target, "plugins/cache/agent-workflows/scw/0.1.0/docs/host-adapter/contract.md"))
  end
end
RUBY
  ...
  [[ "$status" -ne 0 ]] || fail "final contract verification race unexpectedly committed migration"
  assert_contains "$output" "final delivery verification failed"
  assert_contains "$output" "required prompt host adapter contract"
  [[ ! -e "$plugin_root/docs/host-adapter/contract.md" ]] || \
    fail "contract race fixture did not remove the plugin contract"
  for skill in "$ROOT"/skills/*; do
    [[ -d "$skill" ]] || continue
    assert_file "$target/skills/$(basename "$skill")/SKILL.md"
  done
  ruby -rjson -e 'abort unless JSON.parse(File.read(ARGV.fetch(0))).fetch("delivery_mode") == "flat"' \
    "$target/.agent-workflows-install.json"
  [[ ! -e "$target/.agent-workflows-migration-staging" ]] || fail "contract race rollback retained a staging receipt"
  if compgen -G "$target/.agent-workflows-flat-migration-*" >/dev/null; then
    fail "contract race rollback retained a migration quarantine"
  fi
}
