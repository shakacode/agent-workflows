...
- **Mirror an agent lane claim to a visible `agent-claimed` GitHub label (applied on claim, removed on release, daemon-reconciled on expiry) so owned work is visible to humans and agents, while the backend claim and heartbeat TTL stay the source of truth; state the symmetric owned-means-skip decay rule once — humans decay via the stale-assignment sweep, agents via heartbeat TTL, and the sweep skips `agent-claimed` items.**
