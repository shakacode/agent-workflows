...
- **Add a portable, fail-closed `qa-evidence v2` gate for user-visible UI changes: durable reviewer-visible before/after links, non-blank paint inspection, interaction clips or measured substitutes, unfixed negative controls for visual fixes, and repo-seam performance evidence that distinguishes bundle hygiene from a measured metric. GitHub-only runs prepare local artifacts but remain explicitly blocked until a human attachment yields durable GitHub URLs; configured trackers such as Linear or repo artifact stores can host evidence when available. Historical `qa-evidence v1` receipts remain replayable, while current UI audits can require v2 explicitly.** [issue 261](https://github.com/shakacode/agent-workflows/issues/261).
