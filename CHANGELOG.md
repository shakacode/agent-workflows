...
- **Mirror an agent lane claim to a visible claim label (the `agent_claimed_label` seam, default `agent-claimed`; applied on claim, removed on release, daemon-reconciled on expiry) so owned work is visible to humans and agents, while the backend claim and heartbeat TTL stay the source of truth; batch selection and triage skip `agent-claimed` items so the marker is respected, and the symmetric owned-means-skip decay rule is stated once — humans decay via the stale-assignment sweep, agents via heartbeat TTL, and the sweep skips `agent-claimed` items.**
