...
- **Have `$plan-pr-batch` recommend a model and reasoning-effort pair per lane, conservatively escalate uncertain work, collate lanes by assignment without weakening ownership or scheduling boundaries, and preserve roster-unavailable prompts with dispatch-resolved model classes.**
