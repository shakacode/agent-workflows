...
authority, it prepares the complete
...
`hosted_ci_trigger` in `.agents/agent-workflow-operational.yml`, falling back
to `.agents/agent-workflow.yml` when omitted). If the remaining concern is that optimized
