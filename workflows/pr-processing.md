...
4. Do not merge while a current-head relevant review check is queued, in progress, or known to be posting comments asynchronously. Older-head review checks are stale/advisory history; when a stale older-head `claude-review` check/run is the only visible `claude-review` artifact, it can justify a fallback path only when the auto-merge fallback rule below records the current head SHA, stale run/check URL, and evidence from repeated polling that no current-head run is still queued or in progress.
...
- A fallback trigger is recorded in the PR body or a timestamped PR comment before the fallback result is used. The trigger must be one of: no current-head `claude-review` check is available from the Checks API after at least two queries separated by at least 90 seconds; the only visible `claude-review` check/run is for an older head SHA, no current-head run is queued or in progress after the same repeated polling, and the stale run/check is identified by head SHA and run/check URL; or the current-head check failed because of quota exhaustion, hard usage-limit enforcement, provider-reported capacity such as HTTP 503, or persistent HTTP 429 after one 60-second retry.
