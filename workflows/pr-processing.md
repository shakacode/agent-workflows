...
4. Do not merge while a current-head relevant review check is queued, in progress, or known to be posting comments asynchronously. Older-head review checks are stale/advisory history; when a stale older-head `claude-review` check/run is the only visible `claude-review` artifact, it can justify a fallback path only when the auto-merge fallback rule below is fully satisfied: the trigger is recorded in the PR body before the fallback result is used, at least two Checks API polls at least 180 seconds apart show no current-head run queued or in progress, and the current head SHA plus stale run/check URL are identified.
