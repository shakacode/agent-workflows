...
`batch-usage-receipt-v1` window generated by the resolved pr-batch
`bin/batch-usage-receipt` helper. Bind the inline receipt to its canonical
digest and exact durable non-self-attested URI reference. Reject plain,
`UNKNOWN`, self-attested, worker-self-attested, digest/reference-mismatched, or
extra-field evidence. Map batch descendant-inclusive tokens to aggregate,
coordinator self-only plus batch unattributed tokens to coordinator, and each
lane's descendant-inclusive tokens to that lane; require those views to balance
exactly. The first accepted window binds the batch id, coordinator identity and
root, and all planned lane roots. Later windows begin at the prior exclusive
cutoff. Exact replay does not recount; mutation of the same window, gap,
...
overshoot is one already-admitted in-flight boundary per scope and accepted
usage interval; authoritative reconciliation measures it and rejects a wider
envelope.
