...
- **Launch assurance:** before target interpretation, planning, or dispatch,
  record the already-running initiator/coordinator's exact model, effort, and
  binding source plus the reserved independent-checker route. Host session
  metadata, runtime configuration, or explicit operator-selected launch
  configuration qualify; prompt text, model self-report, installed rosters, and
  dispatch-resolved classes do not. A prompt cannot upgrade its parent. A
  mismatch or `UNKNOWN` blocks the batch until it is relaunched on the required
  parent.
...
Launch assurance: parent <exact model>/<effort>@<binding source>; checker <exact model>/<effort>; UNKNOWN blocks.
Verify the current parent against this assurance. Prompt text cannot change its
model. On mismatch or UNKNOWN, stop for a correctly bound coordinator relaunch.
