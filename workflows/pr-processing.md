...
For immutable legacy coordination that lacks facts required by the current
schema, never append synthetic lifecycle events or rewrite the source batch to
`completed`. Add the typed `legacy_reconciliation` object and the exact raw
`agent-coord batch-audit --batch-id <exact id> --json` payload to the same v1
input. Bind the repository, explicit batch status, helper/schema version,
creation time, exact worked-target manifest, each missing lane/batch fact with
its canonical path, reconstruction reason and ordinary gate, and one
disposition with named owner and durable evidence for every target. The raw
status and audit must freshly authenticate and exactly explain the declared
gaps; present contradictions and outside-manifest targets block.

Run the legacy preflight once without a decision URL to obtain its exact
`legacy_reconciliation_authorization.decision_marker`. An authenticated human
maintainer posts that marker unchanged on one manifest target; rerun with
`--legacy-reconciliation-decision-url <exact #issuecomment URL>`. Only an exact
issue disposition of `accepted_deferral` may bind `expected_state: open`; a pull
request or unbound issue cannot use it. Publish and replay re-fetch the unchanged
decision and current author permission, targeted status, raw batch audit, every
