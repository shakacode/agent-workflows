...
- A multi-target group handed off with only the readable prompt is incomplete: copy-paste must also deliver the exact immutable plan-state reference, while host-native delivery must provide either a byte-preserving plan envelope or that same immutable-reference path.
