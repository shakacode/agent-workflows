...
8. Flag review-gate violations:
   ...
   - required QA coverage/scope evidence that was missing, stale, still
     ...
     findings untriaged
9. Flag user-visible changes missing from the repo's changelog. Classify them as
   **Needs changelog update**: user-visible change is missing from the repo's
   changelog; create or reuse one bundled changelog issue as durable tracking
   and recommend `/update-changelog` when useful. Report them in step 11 as:
   Missing changelog candidates, bound to one created or reused bundled
   changelog issue, with a recommendation to run `/update-changelog` when
   useful.
10. Produce a deduped issue plan for non-OK findings:
    ...
    - one bundled changelog issue for missing changelog entries
