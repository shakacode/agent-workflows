...
`--expected-head-sha`; replay filters preserved supersession history to that
head, requires exactly one current-head `qa-evidence-supersession v1` marker,
validates its exact field set, and checks that its head and QA-required
classification match the adjacent replayable
