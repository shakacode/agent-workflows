...
- Finalization is current only when the authenticated finalizer record names the exact current head SHA and score and its timestamp is at or after the PR body's `lastEditedAt`. Any later head or PR-body edit invalidates the finalization and requires a new record from a distinct finalizer before auto-merge.
- Once `Finalized by:` is populated, any later confidence-block edit also has a PR comment with a `Confidence Block Updated:` header, the previous score/finalizer, and the reason for the edit; that comment documents the change but does not replace re-finalization.
