...
- After a successful private claim on an issue/PR, mirror it to the seam's claim
  ...
  — the same visible-hint-not-lock rule as the batch claim step (see the
  `agent-claimed` label-mirror rule in `workflows/pr-processing.md`). Mirror only
  when the backend provides claim-label expiry reconciliation; skip entirely when
