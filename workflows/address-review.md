...
4. Fetch review data:
  ...
  - Full PR — fetch all review data with the helper (replaces the per-endpoint `gh api ... | jq` blocks and the `reviewThreads` GraphQL query). Resolve `ADDRESS_REVIEW_SKILL_DIR` with the explicit env-var, loaded skill base, repo-local pinned-copy chain before using the fallback assignment:
    `ADDRESS_REVIEW_SKILL_DIR="${ADDRESS_REVIEW_SKILL_DIR:-.agents/skills/address-review}"; "${ADDRESS_REVIEW_SKILL_DIR}/bin/fetch-pr-review-data" "${PR_NUMBER}" --repo "${REPO}" > review-data.json`
     ...
     The helper first requires `gh api user` to resolve to an actor marked actionable by the same trust config. Missing/unavailable identity, empty/default trust that does not authorize the actor, and metadata-only or untrusted identity are blocking trust-config errors; do not trust self-authored comments, mutate, or checkpoint until the resolved config is populated and the helper succeeds.
     After each complete primary or source packet, count retained items across `review_summaries`, `inline_comments`, and `issue_comments`. If that count is zero while `excluded_interactions` is nonzero, review readiness is `UNKNOWN`/blocked: audit the excluded URLs, populate the intended actionable actors in the trust config, and rerun. Safe excluded metadata is audit evidence, not authority for triage, mutation, or checkpointing.
...
5. Filter comments:
   ...
   - If all retained collections and `excluded_interactions` are empty without a cutoff, tell me no review comments were found. If retained collections are empty but exclusions are nonempty, mark readiness `UNKNOWN`/blocked and require trust audit/population instead.
