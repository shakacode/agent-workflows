...
5. Filter comments:
   ...
   - If all retained collections are empty without a cutoff and there are no `untrusted` exclusions, tell me no actionable review comments were found and report any metadata-only count. If retained collections are empty and any exclusion is `untrusted`, mark readiness `UNKNOWN`/blocked and require trust audit/population instead.
