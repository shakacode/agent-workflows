...
- In completed-batch mode only:
  ...
  - Use `completed-batch-audit-receipt` for both `publish` and `replay`; `--targets-json` is a JSON array of exact `host`, `repo`, `type` (`pull_request` or `issue`), and positive `number` objects. The `completed-batch-publication-preflight-input` v1 fields are `batch_id`, `expected_targets`, raw `coordination_status`, `target_snapshots`, and `qa_evidence`; a WAIVED row's `maintainer_waiver` contains only its exact `url`. Never substitute a prose/caller summary for the bounded status payload.
