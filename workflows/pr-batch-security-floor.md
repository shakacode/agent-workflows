...
The helper must independently authenticate the ref, pin its Git, GitHub CLI, and SSH executables outside the repository and temporary directories, bind the invoking checkout to the fetched base by comparing tracked files as raw bytes outside clean filters, and reject incomplete or untrusted actors (including the actor who merged the PR), API evidence, policy, repository identity, or ancestry. When it selects a repo-local trust config automatically or through explicit `--trust-config PATH`, reject a PR that changes that configured path, require a regular blob with byte-identical contents at the same repository-relative path in the fetched base, and preserve its fetched source/digest/mode in the receipt; an untracked or locally changed actor policy cannot justify trusted-base acceptance. Rerun preflight after any base movement so the provenance is current.
